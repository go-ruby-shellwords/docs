...
// pass reported as ns/op. Emits the same RESULT/CHECK protocol run.sh consumes.
package main
...
// output. run.sh ignores non-RESULT lines; the verify step diffs these against
// the Ruby side to prove the pure-Go library computes byte-identical results to
// MRI before any timing is trusted.
