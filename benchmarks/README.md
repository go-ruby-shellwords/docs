...
# `go-ruby-shellwords` library-level benchmark harness

Reproducible, cross-runtime benchmark of the **pure-Go `go-ruby-shellwords`
...
as fast as the reference runtime's own `Shellwords`?* Ruby's `shellwords` is a
**pure-Ruby stdlib module** (no C extension), so parity is reachable head-on.

## Layout

- `go/`             — self-contained Go driver; `go.mod` pins the published
  library by pseudo-version (no `replace`). `go/bench` (the built binary) is
  git-ignored.
- `ruby/shellwords.rb` — the equivalent workload; `ruby/_harness.rb` is the shared
...
their per-op outputs are checked identical to MRI (SHA-256) before timing.
Results are published, dated, in `../docs/performance.md`.

## Operations

- **split**  — tokenise a representative command line with quotes, a
  backslash-escaped space and quote-concatenation (`Shellwords.split`).
- **escape** — escape a string full of shell metacharacters
  (`Shellwords.escape`).
- **join**   — escape-and-join an argument vector, including an empty element
  (which becomes `''`) (`Shellwords.join`).
